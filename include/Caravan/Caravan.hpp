...
namespace Caravan{
    ...
    namespace Impl{
        ...
        struct Add{
            ...
            template <typename T, typename... T2s>
            static constexpr auto operator()(Typelist<T2s...>, T){
                return Typelist<T2s..., T>{};
            };
        };
        ...
        struct SubtractOne{
            ...
            constexpr auto operator()(Typelist<T, Ts...>){
                if constexpr (std::is_same<S, T>::value){
                    ...
                    return Add::operator()(T{},SubtractOne<S>::operator()(Typelist<Ts...>{}));
                }
            };
        ...
        template <typename... Ts>
        constexpr std::string toString(Typelist<Ts...>){
            return ((std::string(typeid(Ts).name()) + ", ") + ...);
        };

        template <typename... Ts>
        constexpr std::string toString(Ts...){
            return ((std::string(typeid(Ts).name()) + ", ") + ...);
        };
        ...
        template <typename T>
        constexpr bool contains(Typelist<>){
            return false;
        }

        struct Subset{
            template <typename... T1s, typename... T2s>
            constexpr bool subset(Typelist<T1s...>, Typelist<T2s...>);

            template <typename T1, typename... T1s, typename T2, typename... T2s>
            constexpr bool subset(Typelist<T1, T1s...> tl1, Typelist<T2, T2s...> tl2){
                return contains<T1>(tl2) && subset(Typelist<T1s...>{}, SubtractOne<T1>{}(tl2));
            }
    ...
    template <class T, typename S>
    using subtractOne = decltype(Impl::SubtractOne<S>{}(T{}));
